...
import { WithServerResourceInfo } from "@inrupt/solid-client";
...
export async function getLocations(webID:string) {
  ...
  let locations: Marker[] = []; // initialize array of locations
  let locationPaths; 
  try{
...
export async function getLocationFromDataset(locationPath:string){
  ...
  // let reviews: ReviewType[] = []; // initialize array to store the reviews
   let reviews = await getLocationReviews(datasetPath) // get the reviews
  ...
  let location : Marker = {
      name: name,
      position : {lng: new Number(longitude) as number, lat: new Number(latitude) as number},
      description: description,
      url: url,
      category: category,
      image: locationImages,
      reviews: reviews,
      ratings: new Number(ratings) as number
  }
  return location;
}
...
export async function getLocationReviews(folder:string) {
  ...
  try {
    ...
    for (let review of things) {
      reviews.push(getStringNoLocale(review, SCHEMA_INRUPT.description) as string);
    }
...
//  */
export async function addLocationReview(location:Marker, review:string){
...
//  let url = location.url?.split("#")[0] as string; // get the path of the location dataset
  ...
  let newReview = buildThing(createThing())
    .addStringNoLocale(SCHEMA_INRUPT.description, review)
